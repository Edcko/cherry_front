...
export default function useEmpleados(){
    ...
    const addEmpleado = async (newEmpleado) => {
        ...
        try{
            ...
            app.appContext.config.globalProperties.$showAlert("El empleado se dio de alta correctamente", "success");
        } catch (error){
            console.error(error);
            app.appContext.config.globalProperties.$showAlert("Hubo un error al registrar al empleado.", "error");
        }
    ...
    return {empleados, addEmpleado, updateEmpleado, deleteEmpleado};
}