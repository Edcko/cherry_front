...
export default function useCitas() {
  ...
  const addCita = async (newCita) => {
    ...
    const fechaLimite = new Date(new Date().getFullYear(), 4, 4, 23, 59, 59);

    // Verifica si la fecha de la cita es mayor a la fecha limite
    if (date > fechaLimite) {
      app.appContext.config.globalProperties.$showAlert("Las citas solo se pueden agendar hasta el 04 de Mayo. Espera esa semana para que se abran los días posteriores.", "error");
      return;
    }
