...
export default function useCitas() {
  ...
  const addCita = async (newCita) => {
    ...
    // Fecha limite para agendar citas ( 03 de Agosto del anio actual)
    const fechaLimite = new Date(new Date().getFullYear(), 7, 10, 23, 59, 59);

    // Verifica si la fecha de la cita es mayor a la fecha limite
    if (date > fechaLimite) {
      app.appContext.config.globalProperties.$showAlert("Las citas solo se pueden agendar hasta el 10 de Agosto. Espera esa semana para que se abran los días posteriores.", "error");
      return;
    }
