...
export default function useCitas() {
  ...
  const addCita = async (newCita) => {
    ...
    // Fecha limite para agendar citas ( 01 de Junio del anio actual)
    const fechaLimite = new Date(new Date().getFullYear(), 5, 15, 23, 59, 59);

    // Verifica si la fecha de la cita es mayor a la fecha limite
    if (date > fechaLimite) {
      app.appContext.config.globalProperties.$showAlert("Las citas solo se pueden agendar hasta el 15 de Junio. Espera esa semana para que se abran los días posteriores.", "error");
      return;
    }
